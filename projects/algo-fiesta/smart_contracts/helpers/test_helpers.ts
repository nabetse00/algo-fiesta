import algosdk, { Algodv2, Indexer, Account, encodeUint64, encodeAddress, decodeAddress } from 'algosdk'
...
export async function dispenseAlgos(
  to: Account,
  dispenser: Account,
...
export function decodeTicketTypeBox(boxBytes: Uint8Array): TicketTypeBox {
  const codec = algosdk.ABIType.from('(string,string,uint64,uint64,uint64)')
  const decodedValue = codec.decode(boxBytes)
  const val = decodedValue.toString().split(',')
  const ttb: TicketTypeBox = {
    uri: val[0],
    uriHash: val[1],
    priceMicroAlgo: parseInt(val[2], 10),
    supply: parseInt(val[3], 10),
    maxPerAddress: parseInt(val[4], 10),
  }
  return ttb
}
